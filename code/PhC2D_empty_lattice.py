...
def plot_empty_lattice_dispersion():
    ...
    ax_sqr.plot([0, 0], [0, 5], color="grey", linestyle='dashed')
    ax_sqr.plot([0.5*K_sqr, 0.5*K_sqr], [0, 5], color="grey", linestyle='dashed')

    ax_hex.plot([0, 0], [0, 5], color="grey", linestyle='dashed')
    ax_hex.plot([0.5*K_hex, 0.5*K_hex], [0, 5], color="grey", linestyle='dashed')
    ...
    for m1 in range(-2,2):
        for m2 in range(-2,2):
            ...
            ax_sqr.plot(x, y, color="blue")
              ...
              # hexagonal MG line
            x = np.linspace(-0.25*np.sqrt(3)*K_hex, 0, nump)
            y = cw * dispersion(np.linspace(0.375*K_hex, 0, nump), 
                                np.linspace(0.125*np.sqrt(3)*K_hex, 0, nump), 
                                m1, m2, K1_hex, K2_hex)
            ax_hex.plot(x, y, color="blue")
              # hexagonal GK line
            x = np.linspace(0, 0.5*K_hex, nump)
            y = cw * dispersion(np.linspace(0, 0.5*K_hex, nump), 
                                0, 
                                m1, m2, K1_hex, K2_hex)
            ax_hex.plot(x, y, color="blue")
              # hexagonal KM line
            x = np.linspace(0, 0.25*K_hex, nump) + 0.5*K_hex
            y = cw * dispersion(np.linspace(0.5*K_hex, 0.375*K_hex, nump), 
                                np.linspace(0, 0.125*np.sqrt(3)*K_hex, nump), 
                                m1, m2, K1_hex, K2_hex)
            ax_hex.plot(x, y, color="blue")
    ...
    ax_hex.set_xlim([-0.25*np.sqrt(3)*K_hex, 0.75*K_hex])
    ax_hex.set_ylim([0, 2])
    ax_hex.set_xticks([0, -0.25*np.sqrt(3)*K_hex, 0.5*K_hex, 0.75*K_hex], [r'$\Gamma$', 'M', 'K', 'M'])
    ax_hex.set_yticks([])
